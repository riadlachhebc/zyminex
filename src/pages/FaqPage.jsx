  ...
  return (
    <div className={`faq-item ${open ? 'faq-item--open' : ''}`}>
      <button className="faq-item__trigger" onClick={() => setOpen(!open)}>
        <span className="title-md">{q}</span>
        <span className="material-icons-outlined faq-item__chevron">expand_more</span>
      </button>
      <div className="faq-item__content">
        <p className="body-lg">{a}</p>
      </div>
    </div>
  )
}
    ...
    <div className="faq-page" id="faq-page">
      ...
      <section className="faq-hero" id="faq-hero">
        ...
        <div className="container">
          <span className="label-md" style={{ color: 'var(--primary)' }}>Help Center</span>
          <h1 className="display-lg" style={{ marginTop: 'var(--space-3)' }}>Frequently Asked Questions About IPTV</h1>
          <p className="body-lg" style={{ color: 'var(--on-surface-variant)', maxWidth: '640px', margin: 'var(--space-4) auto 0' }}>
            Everything you need to know about IPTV streaming — how it works, what devices it supports, and what to expect from a premium service.
          </p>
        </div>
      </section>

      <section className="section" id="faq-content">
        <div className="container faq-container">
          {faqs.map((group) => (
            <div key={group.category} className="faq-group">
              <h2 className="headline-md faq-group__title">
                <span className="material-icons-outlined" style={{ color: 'var(--primary)', fontSize: '1.25rem' }}>
                  {group.category === 'General IPTV Questions' ? 'info' : group.category === 'Technical Questions' ? 'settings' : group.category === 'Device Questions' ? 'devices' : 'payments'}
                </span>
                {group.category}
              </h2>
